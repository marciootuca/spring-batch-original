...
public class RealizFuturoPresenter {
    ...
    private static RealizFuturoPresenter toPessoalEnvol(boolean isCredit, RealizFuturoPresenter envol, TransactionOPBKPresenter trans){
        ...
        return Optional.ofNullable(trans)
                .filter(f -> f.getTargetIdentification() != null || f.getOriginIdentification() != null)
                .map(transPresenter -> {
                    envol.setNumCadPssoaEnvol(isCredit
                            ? transPresenter.getOriginIdentification()
                            : transPresenter.getTargetIdentification());
                    envol.setNomTpoPssoaEnvol(transPresenter.getOriginIdentification().length() <= 11
                                    ||  transPresenter.getTargetIdentification().length() <= 11
                                    ? PersonType.PF.getDescription()
                                    : PersonType.PJ.getDescription());
                    envol.setCodInstcEnvol(isCredit ? transPresenter.getOriginEntity() : transPresenter.getTargetEntity());
                    envol.setCodAgEnvol(isCredit ? transPresenter.getOriginBranch() : transPresenter.getTargetBranch());
                    envol.setNumCtaEnvol(isCredit ? transPresenter.getOriginAccount() : transPresenter.getTargetAccount());
                    envol.setNumDigtoCtaEnvol(isCredit
                                    ? transPresenter.getOriginAccount().substring(transPresenter.getOriginAccount().length() -1)
                                    : transPresenter.getTargetAccount().substring(transPresenter.getTargetAccount().length() -1))   ;
                            return envol;
                        }
                ).orElse(envol);
